...
sub normalize_url {
    ...
    if ( ! $uri_next->can( 'host' ) ) {
        if ( $url =~ m{^/(.+)} ) { #if url starts with / ( ie. $uti->host is not defined) 
    ...
    warn "2. HOST   URI CURRENT.: " . $uri_current->host;
            $uri_next = URI->new( $uri_current->host . $uri_current->path . $url );
...
#   warn Dumper $self->url_list and sleep 0;
    ...
    return if exists $self->url_visited->{ $item->{ url } }; #return if not visited
    ...
    $self->browse( $item->{ url }, $item->{ query_params }||undef );       #access page content and loads to $self->content
